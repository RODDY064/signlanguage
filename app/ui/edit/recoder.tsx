    ...
    <div className="w-full h-full top-0 bottom-0 fixed z-[90] bg-black/30 flex items-center justify-center">
      ...
      render={({status, previewStream, startRecording, stopRecording, mediaBlobUrl })=>{
        ...
        return(<div
            ...
            <div className="w-full h-[360px] md:min-w-[800px] md:min-h-[500px] rounded overflow-hidden border  border-black/30 relative p-[2px] ">
            {status === "idle" && <div className="w-full h-full bg-gray-100 flex items-center flex-col gap-3 justify-center">
              <div className="siz-24 border-2 border-black rounded-full flex items-center justify-center">
               ...
               <p className="text-black/50 text-sm">Click start to record</p>
            </div> }
            {recording && status === 'acquiring_media' && <div className="w-full h-full bg-gray-100 flex items-center justify-center ">Loading...</div>}
            {status === "recording" &&  <VideoPreview stream={previewStream}/>}
            {status === "stopped" &&  <VideoPreview media={mediaBlobUrl} setVideo={setVideo}/>}
            </div>
            ...
            {status === 'recording' ?
              <div className="flex gap-2 items-center mt-4 md:mt-0">
                 ...
                 transition={{ repeat:-1}}
                 className="size-4 bg-red-600 rounded-full"></motion.div>
              ...
              <div className="md:w-full flex items-center gap-3">
                ...
                className="py-2 px-6 cursor-pointer bg-sky-300 text-blue-700 flex items-center gap-1 rounded-md">
                     <Image src="/icons/stop.svg" width={26} height={26} alt="stop video"/>
...
const VideoPreview = ({ stream, media , setVideo }: { stream?: MediaStream | null, media?: string | null , setVideo?:any}) => {
  ...
  return (
    <div className="w-full h-full relative">
      {stream ? (
        <video ref={videoRef} width={"800"}  autoPlay controls />
      ) : (
        media && <video width={"800"} src={media} autoPlay   controls />
      )}
    </div>
  );
};
